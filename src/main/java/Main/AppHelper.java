...
public class AppHelper {
        ...
        do {
            ...
            switch (option) {
                ...
                case 6:
                    ...
                    switch (subOption3) {
                        case 1:
                            AppHelper.searchPupilAndParent(pupilManagement, parentManagement, sc);
                            break;
                        case 2:
    ...
    public static boolean isValidAddress(String address) {
        // Allowing for more flexible address formats, including an optional phone
        // number at the end
        String addressRegex = "(\\d+),\\s(.*),\\sPhuong\\s(.*),\\sQuan\\s(.*),\\sThanh pho\\s(.*)(,\\s\\d+)?$|(null)";
    ...
    public static void searchPupilAndParent(PupilManagement pupilManagement, ParentManagement parentManagement,
            Scanner sc) {
        int option = 0;
        do {
            System.out.println("Choose search option:");
            System.out.println("1. Search by pupil name and display parents");
            System.out.println("2. Search relatives by pupil name");
            option = sc.nextInt();

            switch (option) {
                case 1:
                    System.out.print("Enter pupil name to search: ");
                    sc.nextLine(); // Consume the newline character
                    String searchName = sc.nextLine();
                    Pupil pupil = pupilManagement.findPupilsByName(searchName);

                    if (pupil != null) {
                        System.out.println("Pupil: " + pupil.getFullname());

                        // Display parents of the pupil
                        Parent parent = pupil.getParents();
                        if (parent != null) {
                        ...
                        pupilManagement.exportSearchResultToFile();
                    } else {
                        System.out.println("Pupil not found.");
                    }
                    break;
                case 2:
                    System.out.print("Enter pupil name to search relatives: ");
                    sc.nextLine(); // Consume the newline character
                    String relativeSearchName = sc.nextLine();
                    pupilManagement.searchRelativesByName(relativeSearchName);
                    pupilManagement.exportSearchResultToFile();
                    break;
                default:
    ...
    public static String createParentID(String lastParentID) {
        ...
        int number = 0; // Default value if lastParentID is empty

        if (!lastParentID.isEmpty()) {
            number = Integer.parseInt(lastParentID.substring(2));
        }
